...
public class UploadFileRepository implements IUploadFileRepository {
	...
	public void uploadFile(UploadFileVO file) {
		...
		jdbcTemplate.update(sql, file.getFileId(), file.getDirectoryName(), 
				file.getFileName(), file.getFileSize(), file.getFileContentType(),file.getFileData());
	}

	@Override
	public List<UploadFileVO> getFileList(String directoryName) {
		String sql = "SELECT FILE_ID AS fileId, DIRECTORY_NAME AS directoryName, FILE_NAME AS fileName, "
	...
	public UploadFileVO getFile(int fileId) {
		String sql = "SELECT FILE_ID AS fileId, DIRECTORY_NAME AS directoryName, FILE_NAME AS fileName, "
				+ "FILE_SIZE AS fileSize, FILE_CONTENT_TYPE AS fileContentType, FILE_DATA AS fileData "
	...
	public String getDirectoryName(int fileId) {
		String sql = "SELECT DRICTORY_NAME FROM UPLOAD_FILE WHERE FILE_ID=?";
	...
	public void updateFile(UploadFileVO file) {
		String sql = "UPDATE UPLOAD_FILE SET DIRECTORY_NAME=?, FILE_NAME=?, "
				+ "FILE_SIZE=?, FILE_CONTENT_TYPE=?, FILE_DATA=? WHERE FILE_ID=?";
		jdbcTemplate.update(sql, file.getDirectoryName(), file.getFileName(), 
				file.getFileSize(), file.getFileContentType(), file.getFileData(), file.getFileId());
	}
}
